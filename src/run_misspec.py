"""
Misspecification stress-test: fit Pareto/NBD to data with regular (Gamma-k)
inter-purchase times and measure how calibration degrades as regularity k grows.

k=1 is Pareto/NBD itself (calibrated baseline); k>1 violates the exponential-IPT
assumption. We score the (misspecified) Pareto/NBD forecast with randomized PIT
(all customers and forecast-time x>0 subgroup), CRPS, and coverage.

Resumable + time-budgeted (foreground batches). Run:
    python src/run_misspec.py --reps 15 --max-seconds 500
"""
...
def run_one(k, rep, seed):
    ...
    for cond, mask in [("all", np.ones(len(y), bool)), ("xcal_pos", xcal > 0)]:
        ...
        rows.append(dict(
            k=k, rep=rep, cond=cond, n=int(mask.sum()),
            pit_ks=pit_ks(pit),
...
def main():
    ...
    ap.add_argument("--out", default="misspec_results.csv")
    args = ap.parse_args()
    out = RES / args.out
